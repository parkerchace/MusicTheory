/**
 * MUSIC THEORY ENGINE MODULE
 *
 * Core music theory calculations, scales, and chord analysis
 * Used by all other music theory modules
 *
 * Features:
 * - 60+ authentic scales from multiple traditions
 * - Complete chord formula system
 * - Functional harmony analysis
 * - Container chord analysis
...
class MusicTheoryEngine {
    ...
    getScaleNotes(key, scaleType) {
        ...
        try {
            return this.getScaleNotesWithKeySignature(key, scaleType);
        } catch (e) {
    ...
    getScaleNotesWithKeySignature(key, scaleType) {
        ...
        try {
            ...
            if (modalSet.has(String(scaleType))) {
                const parent = this.getParentMajorForMode(key, scaleType);
    ...
    getDiatonicChord(degree, key, scaleType = 'major') {
        ...
        const stacked = this.buildScaleChord(key, scaleType, degree, 4);
