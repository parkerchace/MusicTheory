/**
 * PROGRESSION BUILDER MODULE (2D GENERATIVE)
 *
 * Generative chord progression builder with 2D control system
 * 
 * WEST-EAST (Horizontal): Chord Complexity
 * - West (0): Simple triads (3 notes)
 * - Center (50): 7th chords (4 notes)
 * - East (100): Extended chords - 9th, 11th, 13th (5-7 notes)
 *
 * NORTH-SOUTH (Vertical): Harmonic Adventure
 * - South (0): Conservative - diatonic, functional harmony, smooth voice leading
 * - Center (50): Balanced - some substitutions, secondary dominants
 * - North (100): Adventurous - modal interchange, chromatic mediants, altered chords
 */
...
class ProgressionBuilder {
    ...
    generateChordForDegree(degree, position) {
        const mode = this.state.harmonizationMode || 'melody';
        
        if (mode === 'harmony') {
            // HARMONY MODE: degree represents a chord tone to be contained
            return this.pickChordContainingDegree(degree, position);
        } else {
            ...
            const chosen = this.pickChordByGradeAndComplexity(degree, baseChord);
            return chosen || baseChord; // Fallback to diatonic if no match
        }
    }
        ...
        this.container.innerHTML = `
            <div class="progression-builder-ui">
                        ...
                        <div class="pb-input">
                            <div class="pb-input-numbers">
                                ${this.state.inputNumbers.length > 0 ? 
                                    this.state.inputNumbers.map(n => `<span class="pb-chip">${n}</span>`).join('') : 
                ...
                <div class="pb-generated">
                    <div class="pb-gen-scroll">
                        ${this.state.currentProgression.length > 0 ?
                            this.state.currentProgression.map((chord, i) => {
                                const meta = this.state.progressionMeta[i] || {};
                                const inputDegree = this.state.inputNumbers[i];
                                const tierInfo = this.getGradeTierInfo(meta.gradeTier || this.state.gradeTier);
                                const mode = this.state.harmonizationMode || 'melody';
                                const modeLabel = mode === 'harmony' ? '(contains)' : '(under)';
                                return `
                                    <div class="pb-gen-chip" style="border-left-color:${tierInfo.color}; position: relative;">
                                        <div class="pb-gen-meta">
                                            <span class="pb-badge" title="${mode === 'harmony' ? 'This chord contains degree ' + inputDegree : 'Melody note: degree ' + inputDegree}">${typeof inputDegree !== 'undefined' ? inputDegree : ''} ${modeLabel}</span>
                                            <span class="pb-tier" style="color:${tierInfo.color};" title="${tierInfo.label}">${tierInfo.short}</span>
                                        </div>
                                        <div class="pb-chord" style="cursor: pointer;" data-index="${i}" title="Click for substitutions">${chord}</div>
                                        <button class="pb-sub-btn" data-index="${i}" title="Show substitution options" style="position: absolute; bottom: 4px; right: 4px; font-size: 0.65rem; padding: 2px 6px; border: 1px solid rgba(148,163,184,0.3); background: rgba(15,23,42,0.8); color: #cbd5e1; border-radius: 4px; cursor: pointer;">🔄</button>
                                    </div>
                                `;
                            }).join('') :
                            '<div class="pb-empty">No progression yet - generate numbers to start</div>'
                        }
                    </div>
                </div>
