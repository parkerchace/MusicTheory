/**
 * @module ContainerChordTool
 * @description Specialized tool for finding chords that contain specific notes with advanced filtering
 * @exports class ContainerChordTool
 * @feature Input for multiple notes
 * @feature Scale/key context awareness
 * @feature Chord grading system (★★★ Perfect, ★★ Excellent, ★ Good)
 * @feature Detailed chord information display
 * @feature Piano visualization integration
 * @feature Filter by complexity (triads, sevenths, extended)
 */
...
class ContainerChordTool {
    constructor(musicTheoryEngine) {
        ...
        this.state = {
            ...
            openGroups: new Set(),
            // Compact mode for multi-note results: show Top matches and previews instead of full long lists
            compactMultiMode: true
        };
    ...
    const sortedRoles = Array.from(groups.keys()).sort((a,b)=> getProgressionScore(b) - getProgressionScore(a));
        ...
        const renderMulti = () => {
            ...
            const compact = !!this.state.compactMultiMode;
            // Top matches: only in compact mode
            const topMatches = compact ? (filteredResults || []).slice(0, 8) : [];
            return `
                <div class="multi-groups">
                    ${topMatches.length ? `
                        <div class="top-matches" style="margin-bottom:8px;">
                            <div style="font-weight:700; margin-bottom:6px;">Top matches</div>
                            <div style="display:flex; flex-wrap:wrap; gap:6px">${topMatches.map(ch => renderChordCard(ch)).join('')}</div>
                        </div>
                    ` : ''}
                    ${grouped.map(([label, entries]) => {
                        const open = this.state.openGroups.has(label);
                        const previewCount = 4;
                        const previewHtml = entries.slice(0, previewCount).map(({chord}) => renderChordCard(chord)).join('');
                        const fullHtml = entries.map(({chord}) => renderChordCard(chord)).join('');
                        return `
                            <div class="mg-section" data-group-label="${label}">
                                ...
                                <div class="mg-content" data-label="${label}">
                                    ${compact ? (open ? fullHtml : previewHtml) : fullHtml}
                                    ${compact && !open && entries.length > previewCount ? `<div style=\"margin-top:6px\"><button class=\"btn role-show-more\" data-role=\"${label}\">Show ${entries.length - previewCount} more</button></div>` : ''}
                                    ${compact && open && entries.length > previewCount ? `<div style=\"margin-top:6px\"><button class=\"btn role-show-more\" data-role=\"${label}\">Show less</button></div>` : ''}
                                </div>
                            </div>
                        `;
                    }).join('')}
                ...
                <div class="results-area">
                    ...
                    ${isMulti ? `
                        <div class="multi-group-controls" style="display:flex; gap:6px; align-items:center; flex-wrap:wrap; margin:6px 0 8px 0;">
                            ...
                            <label style="font-size:0.75rem; color:var(--text-secondary); display:flex; align-items:center; gap:4px; margin-left:6px;">
                                <input type="checkbox" id="mg-compact-cb" ${this.state.compactMultiMode ? 'checked' : ''} /> Compact view
                            </label>
                        </div>
                    ` : ''}
        ...
        this.containerElement.querySelectorAll('.mg-toggle').forEach(btn => {
            btn.addEventListener('click', () => {
                ...
                if (this.state.openGroups.has(label)) {
                    this.state.openGroups.delete(label);
                } else {
                    this.state.openGroups.add(label);
                }
                this.render();
            });
        });

        // Show more / Show less handlers for compact group previews
        this.containerElement.querySelectorAll('.role-show-more').forEach(btn => {
            btn.addEventListener('click', () => {
                const role = btn.getAttribute('data-role');
                if (!role) return;
                if (this.state.openGroups.has(role)) {
                    this.state.openGroups.delete(role);
                } else {
                    this.state.openGroups.add(role);
                }
                this.render();
            });
        });

        // Compact mode checkbox handler
        const compactCb = this.containerElement.querySelector('#mg-compact-cb');
        if (compactCb) {
            compactCb.addEventListener('change', () => {
                this.state.compactMultiMode = !!compactCb.checked;
                // When toggling modes, reset openGroups for consistency
                this.state.openGroups = new Set();
                this.render();
            });
        }
